...
List<HashMap<String,Object>> list = null;
    if(request.getAttribute("borrowList")!=null){
    	list = (List<HashMap<String,Object>>)request.getAttribute("borrowList");
    }
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <strong class="card-title">待归还图书</strong>
                            </div>
                            <div class="card-body">
                  ...
                  %>
                                        <tr>
                                            ...
                                            <td><%=list.get(i).get("publish") %></td>
                                            <td><%=list.get(i).get("borrowStatus") %></td>
                                            <td><%=sdf2.format(list.get(i).get("borrowDate")) %></td>
                                            <td><%=sdf2.format(list.get(i).get("returnDate")) %></td>
                                            <td>
                                            
                                            <a href="<%=basePath%>main/bookDetail/<%=list.get(i).get("bookId")%>" >详情</a>
                                            <a href="#" id="returnBook<%=list.get(i).get("borrowId")%>" >确认还书</a>
<%--                                              <a href="<%=basePath%>admin/borrowBook/<%=list.get(i).get("bookId")%>?userId=<%=list.get(i).get("userId") %>" >预约确认</a> --%>
<%--                                         
...
<script type="text/javascript">
     ...
     $("a[id^='returnBook']").bind("click",function(){
  		
		 if(confirm("确定要归还这本书吗？")){
		 
		 var id = $(this).prop("id").substring(10);
		 var url = "<%=basePath%>admin/returnBook";
		 var data = "borrowId="+id;
		 $.ajax({
		       	type : "POST",
		       	url : url,
		       	data : data,
		       	...
		       	success : function(data){
		       		if(data.msg=="SUCCESS"){
		       			alert("还书成功！！");
		       			window.location = "<%=basePath%>admin/returnBookList";
		       		}else{
		       			alert(data.msg);
		       		}
		      	}
