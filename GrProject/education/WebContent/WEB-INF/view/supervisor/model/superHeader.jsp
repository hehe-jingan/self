...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	if(request.getSession().getAttribute("supervisor")==null){
		...
		userLoginFlag = "true";
		user = (Supervisor)request.getSession().getAttribute("supervisor");
...
// 	  getMessageCount();
	  if(userIsLoginFlag=="false"){
		  alert("请重新登录！！");
		  window.location.href = "<%=basePath%>supervisor/login";
	  }
	  
	  $("#logoutSystem").click(function(){
		  
		 if(confirm("确定要退出系统吗？？")){
			 window.location.href = "<%=basePath%>supervisor/login?flag=logout";
		 }
